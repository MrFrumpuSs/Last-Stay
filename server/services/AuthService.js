...
import password from "password-gen-v1"
...
class AuthService {
    ...
    async callback(params) {
        if(params.code) {
            ...
            const pass = password.newPassword(6, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890');
            ...
            if(rows.length === 0) {
                const [rows, fields] = await promise.execute(sql2, [response2.data.response[0].id, response2.data.response[0].first_name, response2.data.response[0].last_name, pass, 0]);
                return {id: response2.data.response[0].id, fname: response2.data.response[0].first_name, sname: response2.data.response[0].last_name, code: pass};
            }
